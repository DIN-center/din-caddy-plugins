...
func (s *SolanaClient) GetLatestBlockNumber(httpUrl string, headers map[string]string, auth auth.AuthClient) (int64, int, error) {
	// Send the POST request to get the latest block
	payload := []byte(`{"jsonrpc":"2.0", "method":"getBlockHeight","params":[],"id":1}`)
	// Send the POST request
	resBytes, statusCode, err := s.HTTPClient.Post(httpUrl, headers, []byte(payload), auth)
	if err != nil {
