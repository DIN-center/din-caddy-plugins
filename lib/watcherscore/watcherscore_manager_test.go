...
func TestWatcherScoreManager(t *testing.T) {
	...
	t.Run("empty manager returns WatcherScore with no value", func(t *testing.T) {
		rm := NewEmpty(zaptest.NewLogger(t))
	...
	t.Run("manager with a single network formula containing a single metric generator for a single provider returns WatcherScore with value 0.95", func(t *testing.T) {
	...
	t.Run("manager with a single network formula containing two metric generators for a single provider returns WatcherScore with value 0.915", func(t *testing.T) {
		logger := zaptest.NewLogger(t)
		rm := NewEmpty(logger)
		rm.AddNetworkFormula("gyro", ScoreFormula{
			network: "gyro",
	...
	t.Run("manager update scores smoothly when the metrics changes abruptly over time (using EWMA transform)", func(t *testing.T) {
		...
		rm.AddNetworkFormula("gyro", ScoreFormula{
			...
			scoreTransformer: NewCompositeTransformer(NewEWMATransformer(0.7, logger), NewDefaultHighPassThroughTransformer(logger)),
		})
		//Run scores once
		rm.ComputeScores()
