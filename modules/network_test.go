...
func TestBlockNumberDeltaHealthCheck(t *testing.T) {
	...
	tests := []struct {
		...
		provider         *provider
		blockNumber      int64
		networkBlockNum  int64
		blockNumberDelta int64
		want             HealthStatus
		expectUnhealthy  bool
	}{
		{
			name:         "block number within delta range",
			providerName: "provider1",
			provider: &provider{
				healthStatus: Healthy,
			},
			blockNumber:      5000010,
			networkBlockNum:  5000000,
			blockNumberDelta: 20,
			want:            Healthy,
			expectUnhealthy: false,
		},
		{
			name:         "block number too far ahead",
			providerName: "provider1",
			provider: &provider{
				healthStatus: Healthy,
			},
		...
		{
			name:         "block number too far behind",
			providerName: "provider1",
			provider: &provider{
				healthStatus: Healthy,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			n := &network{
				latestBlockNumber: tt.networkBlockNum,
				BlockNumberDelta: tt.blockNumberDelta,
				logger:          zap.NewNop(),
				Providers: map[string]*provider{
					"provider1": {host: "provider1"},
				},
				CheckedProviders: map[string][]healthCheckEntry{
					"provider1": {{blockNumber: tt.networkBlockNum, timestamp: &timeNow}},
				},
			}

			if tt.name == "single provider - always healthy" {
				// Only include one provider for this test case
				n.Providers = map[string]*provider{
					"provider1": {host: "provider1"},
				}
			} else {
				// Add multiple providers for other test cases
				n.Providers = map[string]*provider{
					"provider1": {host: "provider1"},
					"provider2": {host: "provider2"},
				}
			...
			result := n.blockNumberDeltaHealthCheck(tt.providerName, tt.provider, tt.blockNumber)
			...
			if tt.provider.healthStatus != tt.want {
				t.Errorf("blockNumberDeltaHealthCheck() health status = %v, want %v", tt.provider.healthStatus, tt.want)
			}
		})
	}
}
