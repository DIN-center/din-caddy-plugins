...
func TestSyncMiddlewareWithLatestScores(t *testing.T) {
	...
	mockReputationScoreManager := rs.NewMock(logger, map[string]map[string]*rs.Score{
		"test-network": {
			"provider1": oldScore,
		},
	...
	dinMiddleware := &DinMiddleware{
		...
		DingoClient:                         mockDingoClient,
		testMode:                            true,
		reputationScoreManager:              mockReputationScoreManager,
	...
	tests := []struct {
		...
		{
			name:                                "Sync should update (as block difference is equal to or exceeds epoch 50) but not smart routing sync",
			registryLastUpdatedEpochBlockNumber: uint64(40),
			latestBlockNumber:                   uint64(50),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dinMiddleware.registryLastUpdatedEpochBlockNumber = tt.registryLastUpdatedEpochBlockNumber
			dinMiddleware.SmartRoutingSyncEnabled = tt.smartRoutingSyncEnabled
			mockDingoClient.EXPECT().GetLatestBlockNumber().Return(tt.latestBlockNumber, nil).Times(1)
			// Check if update was called as expected
			if tt.expectedUpdateCall {
				mockDingoClient.EXPECT().GetRegistryData().Return(&din.DinRegistryData{}, nil).Times(1)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dinMiddleware := &DinMiddleware{
				...
				RegistryEnabled:        true,
				SmartRoutingEnabled:    true,
				reputationScoreManager: rs.NewEmpty(logger),
			}
			...
			if tt.expectedError != nil {
				...
				if network != nil {
					...
					// Verify the network is added to the reputation score manager
					assert.NotNil(t, dinMiddleware.reputationScoreManager.GetNetworkFormula(network.Name))
				}
			}
		})
	}
